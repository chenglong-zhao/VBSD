...
parser.add_argument('--noise-factor', type=float, default=0.4, help='gassian noise variance factor')
...
def attack(model, img, label, eps=8/255, iters=10, step=2/255):
    ...
    y_int = label_smoothing(y, 0.1)
    d1 = (x - x_nat).reshape(x.shape[0], -1)  # 256,3072
    d2 = noise.reshape(x.shape[0], -1)   # 256,3072
    d1 = torch.norm(d1, dim=-1, keepdim=True)  # 256,1
    d2 = torch.norm(d2, dim=-1, keepdim=True)  # 256,1

    y = (d1/(d1+d2))*y_int + (d2/(d1+d2))*y_nat
...
sys.stdout = Logger(os.path.join(args.save_dir, 'resnet34_bayes_noise_factor_{}.txt'.format(args.noise_factor)))
...
if args.resume=='finetune':
    ...
    filename = '../checkpoint/resnet34_vallina.pth'
...
def train(epoch):
    ...
    torch.save(model.state_dict(), '../checkpoint/resnet34_bayes_noise_factor_{}.pth'.format(args.noise_factor))
